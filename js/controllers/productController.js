...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    form.addEventListener("submit", async (e) => {
        ...
        if(file){
            try{
                // Aquí subimos la imagen al servidor
                const response = await uploadImageToFolder(file, "products");

                //alert(response + " " + response.url);
                if(response && response.Url) {
                    finalImageUrl = response.Url; // Usamos la URL de la imagen subida
                } else {
    ...
    try {
      ...
      items.forEach((item) => {
        ...
        if (item.image_URL) {
          ...
          img.className = "thumb";
          img.alt = "img";
          img.src = item.image_URL; // Opcional: validar dominio aquí
  ...
  async function cargarCategorias() {
    try{
        ...
        categories.forEach((c) => {
            ...
            options.value = c.idCategoria;
            options.textContent = c.nombreCategoria;
            options.title = c.descripcion;
...
function setFormulario(item) {
    ...
    if(imageUrlHidden){
        imageUrlHidden.value = item.imagen_url || "";
    }
    if(imagePreview){
...
function renderPagination(current, totalPages) {
    ...
    cargarProductos(); // Cargamos los productos al iniciar
});